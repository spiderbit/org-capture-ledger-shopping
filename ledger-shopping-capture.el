...
(defun sbit/ledger-shopping-capture-dialog ()
  (let* ((map-file "~/.emacs.d/shop-items.txt"))
    (let* ((shops (if (file-exists-p map-file)
		      (read-from-file2 map-file) '()))
	   (names (cl-mapcar 'car shops))
	   (shop-name (funcall shopping-completing-function
			       "Shop: " names))
      ...
      (while (let* ((names (cl-mapcar 'car products))
     		    (name (funcall shopping-completing-function
				   "Name: " names))
	...
	(print-to-file2 map-file combined-shops)
	(with-temp-buffer
